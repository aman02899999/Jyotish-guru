import { NextResponse } from "next/server";
import { z } from "zod";
import { getCurrentUser, toSafeUser } from "@/lib/current-user";
import { prisma } from "@/lib/prisma";
import { SUBSCRIPTION_TIERS } from "@/lib/subscriptions";
import { grantSubscription } from "@/lib/grants";
import { isRazorpayConfigured } from "@/lib/razorpay";

const subscribeSchema = z.object({ tierId: z.string() });

/**
 * Mock-activates a subscription tier - no real payment gateway. Only
 * reachable when Razorpay isn't configured; once it is, subscribing goes
 * through /api/payments/razorpay instead and this route refuses to run so
 * it can't be used as a free-unlock bypass.
 */
export async function POST(request: Request) {
  const user = await getCurrentUser();
  if (!user) return NextResponse.json({ error: "Unauthorized" }, { status: 401 });

  if (isRazorpayConfigured) {
    return NextResponse.json({ error: "Use real checkout - the sandbox payment flow is disabled." }, { status: 403 });
  }

  const parsed = subscribeSchema.safeParse(await request.json().catch(() => null));
  if (!parsed.success) {
    return NextResponse.json({ error: "Invalid input" }, { status: 400 });
  }

  const tier = SUBSCRIPTION_TIERS.find((t) => t.id === parsed.data.tierId);
  if (!tier) return NextResponse.json({ error: "Unknown subscription tier" }, { status: 404 });

  await grantSubscription(user.id, tier.id);

  const updatedUser = await prisma.user.findUniqueOrThrow({ where: { id: user.id } });
  const safeUser = toSafeUser(updatedUser);
  return NextResponse.json({ user: safeUser });
}
